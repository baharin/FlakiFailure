...
def CalculateInconclusiveness(coveredpoints, technique):
  ...
  for col in cols: #iterate over runs
...
models = ['APSNG', 'Dave2', 'R1', 'R2', 'R4', 'R3', 'Router', 'Aircraft']
...
for w in withorwithout:
  ...
  for model in models:
    ...
    for t in ts:
      ...
      for theta in theta_values:
        ...
        coveredpoints = pd.read_excel('...\\path_to_file\\GP-'+t+'-coveredpoints-'+model+ '-theta' + theta + '-testset.xlsx', sheet_name = w)

        indexx = results.shape[0]

        dtinconclu = CalculateInconclusiveness(coveredpoints, 'gp'+t)

        dtaccuracy, dtfalsepasses, dtfalsefails = CalculatePerformance(coveredpoints, 'gp'+t, dtinconclu, model)
  ...
  if w == 'dataset1':
    ...
    with pd.ExcelWriter('GPAccuracyInconclu.xlsx', engine='openpyxl', mode='w') as writer:
      results.to_excel(writer, sheet_name=w, index=False)
  
  else:

    with pd.ExcelWriter('GPAccuracyInconclu.xlsx', engine='openpyxl', mode='a') as writer:
      results.to_excel(writer, sheet_name=w, index=False)

    ...
    for ind, t in enumerate(ts):
        for hh in range(0, 10):
            ...
            if model == 'APSNG':
                ...
                offset = 90
            # testset = pd.read_excel('roaddata - beamng - tol is 0.85 - size 2000 with rep - ambiegen - withpassfail.xlsx')
            # testset = PreprocessWithoutRepetition(testset, 10, model)
            ...
            file_exists = os.path.isfile('GP-'+t+'-coveredpoints-'+model+'-theta'+str(theta)+'-testset.xlsx')

            with pd.ExcelWriter('GP-'+t+'-coveredpoints-'+model+'-theta'+str(theta)+'-testset.xlsx', engine = 'openpyxl', mode = 'a' if file_exists else 'w') as writer:

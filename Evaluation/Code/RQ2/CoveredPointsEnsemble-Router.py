...
def CalculatePerformance(asserts, probs, dataset, res, alg):
  ...
  for ii in range(len(asserts)):
    ...
    for jj in range(len(asserts[ii][0])): #iterate over fail rules
      ...
      dt = False
      try:
        rule = ast.literal_eval(asserts[ii][0][jj])
    ...
    for jj in range(len(asserts[ii][1])): #iterate over pass rules
      ...
      dt = False
      try:
        rule = ast.literal_eval(asserts[ii][1][jj])
        ranges = ExtractRanges(rule)
        dt = True
      except:
...
for theta in thetas:
  for hh in range(0, 10):
    ...
    file_exists = os.path.isfile('ensemble-coveredpoints-Router-theta'+str(theta)+'-testset.xlsx')

    with pd.ExcelWriter('ensemble-coveredpoints-Router-theta'+str(theta)+'-testset.xlsx', engine = 'openpyxl', mode = 'a' if file_exists else 'w') as writer:

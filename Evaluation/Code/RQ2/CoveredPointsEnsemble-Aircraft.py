...
def remove_vertices(graph, part_A, part_B, weights):
    ...
    while vertices:
        ...
        while len(list(graph[vertex.id])) > 0:  
...
for theta in thetas:
  ...
  for m, model in enumerate(['R121']):
    ...
    GPdata = pd.read_excel('path_to_file\\GPruleswithProb.xlsx')
    GPdata = GPdata[m: m+126]
    GPdata = GPdata.reset_index(drop=True)
        
    
    dtdata = pd.read_excel('path_to_file/DT-Aircraft.xlsx')
    drdata = pd.read_excel('path_to_file/DR-Aircraft.xlsx')

def ExtractRanges(rule, model):
  ...
  if model == 'APSNG':
    ...
    # dictt = {'CLEV': [0,1], 'SUNO': [0,1], 'SUEV': [0,1], 'FOMO': [0,1], 'FONI': [0,1], 'SUNN': [0,1], 'RAIN': [0,1], 'ARG1': [5, 100], 'ARG2': [30, 40]}
    dictt = {'CLEV': [0,1], 'SUNO': [0,1], 'SUEV': [0,1], 'FOMO': [0,1], 'FONI': [0,1], 'SUNN': [0,1], 'RAIN': [0,1], 'ARG0':[0, 6], 'ARG1': [5, 100], 'ARG2': [30, 85]}

  elif model == 'dave2':

    # dictt = {'CLEV': [0,1], 'SUNO': [0,1], 'SUEV': [0,1], 'FOMO': [0,1], 'FONI': [0,1], 'SUNN': [0,1], 'RAIN': [0,1] , 'ARG1': [5, 20], 'ARG2': [30, 40]}
...
def CalculatePerformance(asserts, probs, dataset, model, res, alg):
  ...
  for ii in range(len(asserts)):
    ...
    for jj in range(len(asserts[ii][0])): #iterate over fail rules
      ...
      print('this is rule in fail part', rule)
    ...
    for jj in range(len(asserts[ii][1])): #iterate over pass rules
      ...
      print('this is rule in pass part', rule)
      ranges = ExtractRanges(rule, model)
...
import pandas as pd
import ast
import ConsistencyCheckingADS


model = 'beamng-town-R2toR4'
...
for hh in range(0, 10):
  for theta in thetas:
    ...
    testset = pd.read_excel('...\\path_to_file\\APTWN_R2_to_R4_testset.xlsx')
    ...
    asserts, probs = GetFailAndPassAssertions(assertions[150:179].reset_index(), theta)
    ...
    print('***********')
    res = CalculatePerformance(asserts, probs, testset, model, res, 'Ensemble')

    import os 

    file_exists = os.path.isfile('ensemble-coveredpoints-ultra-theta'+str(theta)+'-testset.xlsx')

    with pd.ExcelWriter('ensemble-coveredpoints-ultra-theta'+str(theta)+'-testset.xlsx', engine = 'openpyxl', mode = 'a' if file_exists else 'w') as writer:

...
results = pd.DataFrame(columns=['Run', 'TestDataset', 'PopulationSize', 'NumberofGenerations',
                                'MaxTreeDepth', 'BestIndividualFormula', 'BestIndividualFitness', 'BaselinePrecision'])
...
wholepopulation = pd.read_excel('AP_R12_1.xlsx')
...
results.to_excel('GPresults_passclass_aircraft_'+fitness+'.xlsx')

bestfitnessovertime.to_excel(
    'BestFitnessesOvertime_passclass_aircraft_'+fitness+'.xlsx')

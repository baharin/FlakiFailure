...
def GetConfSamp(dictt, passorfail, data):
  ...
  for i in range(len(data.index)):
    ...
    if count == len(dictt): #all the variables are in the ranges
      ...
      if data.loc[i, col] == 0 and passorfail == 'fail':
          trues = trues + 1

      elif data.loc[i, col] == 1 and passorfail == 'pass':
...
for hh in range(1, 10):
  ...
  features  = ['Weather', 'MaxSpeed', 'Traffic Amount']
  ...
  for i in range(20):
    ...
    with open(a+'_fail_decisionrule_dataset'+str(hh)+'_run'+str(i)+'.pkl', 'wb') as file:
  ...
  for i in range(20):
    ...
    with open(a+'_pass_decisionrule_dataset'+str(hh)+'_run'+str(i)+'.pkl', 'wb') as file:
      pickle.dump(clf, file)
  ...
  res.to_excel("DR-"+a+"-dataset"+str(hh)+".xlsx")

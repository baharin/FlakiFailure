...
for kk in range(1, 10):
  ...
  dtassertind = GetFailAndPassAssertions(dtind, 0)
  dtassertsum = GetFailAndPassAssertions(dtsum, 0)
  drassertind = GetFailAndPassAssertions(drind, 0)
  drassertsum = GetFailAndPassAssertions(drsum, 0)
  ...
  if file_exists:
      with pd.ExcelWriter('ensembleRouter.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
          res.to_excel(writer, sheet_name='dataset'+str(kk), index=False)

  else:
      with pd.ExcelWriter('ensembleRouter.xlsx', engine='openpyxl', mode = 'w')  as writer:
          res.to_excel(writer, sheet_name='dataset'+str(kk), index=False)  

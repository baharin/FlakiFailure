#This code adds additional functionalities to the executer code from https://github.com/sbft-cps-tool-competition/cps-tool-competition/tree/main
...
class AbstractTestExecutor(ABC):
    ...
    def execute_test(self, the_test, weather):
        ...
        minimumoob_distance = -10000
        features = {"MAX_CURV": 10000}
        chosenweather = 'NULL'
        alloob_distances = []
        ...
        if is_valid:
            ...
            try:
                test_outcome, description, execution_data, minimumoob_distance, alloob_distances = self._execute(the_test, weather)
            finally:
        ...
        print("the test outcome was ", test_outcome)
        # Mark that generation is restarted.
        self.start_generation_time = time.perf_counter()

        return test_outcome, description, execution_data, minimumoob_distance, features, alloob_distances
    ...
    def _close(self):
        if not self.time_budget.is_over():
            log.warning("Deeite the time budget is not over executor is exiting!")

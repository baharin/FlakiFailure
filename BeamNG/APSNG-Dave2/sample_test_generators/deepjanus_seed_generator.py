# This code adds additional functionalities to the deepjanus_seed_generator code from https://github.com/sbft-cps-tool-competition/cps-tool-competition/tree/main
import random
...
from code_pipeline.test_analysis import max_curvature
import math
import numpy as np
...
class OurRandomJanusGenerator():

    def __init__(self, executor=None, map_size=None, weather = None, maxspeed = None, max_angle = None):
    ...
    def start(self):
        ...
        while(test_outcome == "INVALID" and tried < 20):

            try:
                shutil.rmtree('...\\path_to_file\\BeamNG.tech.v0.26.2.0_user\\0.26')
                os.makedirs('...\\path_to_file\\BeamNG.tech.v0.26.2.0_user\\0.26', exist_ok=True)
            except:
                pass
            ...
            the_test, interpolated_pointss = RoadTestFactory.create_road_test(road_points)

            test_outcome, description, execution_data, minimumoob_distance, features, alloob_distances = self.executor.execute_test(the_test, self.weather)
            ...
            if test_outcome != 'INVALID':
                ...
                #open that excel file and write the info about the road
                roaddata = pd.read_excel(r'...\path_to_code\code_pipeline\roaddata.xlsx')
                roaddata.loc[roaddata.shape[0]] = [0, road_points, interpolated_pointss, minimumoob_distance, features["MAX_CURV"], self.weather, self.maxspeed, MAX_ANGLE, test_outcome, '']
                roaddata.to_excel(r'...\path_to_code\code_pipeline\roaddata.xlsx', index=False)

                alloobfile = pd.DataFrame(columns=['TestIndex', 'Rep', 'OOB'])

                testindex = pd.read_excel(
                    r'...\path_to_code\code_pipeline\roaddata.xlsx')
                testindex = len(testindex.index)

                for hh in range(len(alloob_distances)):
                    alloobfile.loc[alloobfile.shape[0]] = [testindex, 0, alloob_distances[hh]]

                
                for kk in range(9):
                    test_outcome, description, execution_data, minimumoob_distance, features, alloob_distances = self.executor.execute_test(the_test, self.weather)
                    
                    roaddata = pd.read_excel(
                        r'...\path_to_code\code_pipeline\roaddata.xlsx')
                    roaddata.loc[roaddata.shape[0]] = [0, road_points, interpolated_pointss, minimumoob_distance,
                                                       features["MAX_CURV"], self.weather, self.maxspeed, MAX_ANGLE, test_outcome, '']
                    roaddata.to_excel(
                        r'...\path_to_code\code_pipeline\roaddata.xlsx',
                        index=False)

                    for hh in range(len(alloob_distances)):
                        alloobfile.loc[alloobfile.shape[0]] = [testindex, kk + 1, alloob_distances[hh]]


                tried = 21

                alloobfile.to_excel(r'...\outputs\oobsoftest_' + str(testindex) + '_DeepJanus' + '.xlsx')
...
class OurAmbieGenGenerator():
    import sample_test_generators.swat_gen_mo.Optimize as optim
    def __init__(self, executor=None, map_size=None, weather = None, maxspeed = None, max_angle = None):
        self.executor = executor
        self.map_size = map_size
        self.weather = weather
    ...
    def start(self):
        ...
        while(test_outcome == "INVALID" and tried < 20):
            ...
            road_points = cases[case][0]
            angleofroadpoints = cases[case][1]

            mangle = 0.0

            for i in range(len(angleofroadpoints)):

                if angleofroadpoints[i][0] != 'straight':

                    if angleofroadpoints[i][1] > mangle:
                        mangle = angleofroadpoints[i][1]
            ...
            if test_outcome != 'INVALID':
                ...
                #open that excel file and write the info about the road
                roaddata = pd.read_excel(r'...\path_to_file\code_pipeline\roaddata.xlsx')
                roaddata.loc[roaddata.shape[0]] = [0, road_points, interpolated_pointss, minimumoob_distance, features["MAX_CURV"], self.weather, self.maxspeed, mangle, test_outcome, '']
                roaddata.to_excel(r'...\path_to_file\code_pipeline\roaddata.xlsx', index=False)

                alloobfile = pd.DataFrame(columns=['TestIndex', 'Rep', 'OOB'])

                testindex = pd.read_excel(
                    r'...\path_to_file\code_pipeline\roaddata.xlsx')
                testindex = len(testindex.index)
                ...
                for kk in range(9): 
                    ...
                    roaddata = pd.read_excel(
                        r'...\path_to_file\code_pipeline\roaddata.xlsx')
                    roaddata.loc[roaddata.shape[0]] = [0, road_points, interpolated_pointss, minimumoob_distance,
                                                       features["MAX_CURV"], self.weather, self.maxspeed, mangle, test_outcome, '']
                    roaddata.to_excel(
                        r'...\path_to_file\code_pipeline\roaddata.xlsx',
                        index=False)
                ...
                alloobfile.to_excel(r'...\path_to_file\outputs\oobsoftest_'+str(testindex)+'_Ambiegen'+'.xlsx')
